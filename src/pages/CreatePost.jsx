      ...
      try {
        ...
        if (res.error) {
          ...
          if (code === "billing_hard_limit_reached") {
            alert(
              "Sorry! your platform ai credits have been reached!, please contact support or add your own new secure api keys through profile"
            );
            return;
          }
        }
        setForm({ ...form, photo: `res:image/jpeg;base64,${res.photo}` });
      } catch (error) {
        console.log(error);
      ...
      if (submitting) {
        ...
        if (generateAllowed) {
          ...
          if (form.prompt) {
            ...
            alert(
              "Please enter all details before sharing with community with a generated image"
            );
            setSubmitting(true);
            generateAllowed = true;
          }
      ...
      <section className="max-w-7xl mx-auto mb-28">
        <div className="">
          ...
          <p className="mt-2 text-[#666e75] text-[16px] max-[50px]">
            {" "}
            Create imaginative and visually stunning images through the DALL-E
            AI!
          </p>
        </div>
        {loading ? (
          ...
          <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
            ...
            <div className="mt-5 flex gap-5">
              <button
                className="text-rose-100 bg-teal-700 font-medium rounded-md text-sm sm:w-auto p-3 w-full px-5 py-2.5"
                type="button"
            ...
            <div className="mt-10">
              <p className="mt-2 text-[#666375] text-[14px] ">
                You have created the image you want, you can share it with
                others in the community
              </p>
              <button
                className={
                  submitActive
                    ? "mt-3 text-white bg-[#809475] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center shadow-xl transition-all ease-in-out hover:text-[15px] hover:text-[#c0c39e]"
                    : "mt-3 text-gray-300 bg-[#acb9a8] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center"
                }
              >
